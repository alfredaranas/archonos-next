...
**Status:** All milestones 0.5–6 complete. 130 tests passing. Zero runtime dependencies.
...
**130 tests** in `tests/` — all pass in ~8s. Breakdown:
- `test_gate_m1.py` 12 (kernel + schema-shape guards)
- `test_gate_m2.py` 19 (knowledge + the <200ms search gate + the abstract-shape regression test)
...
**No provider?** All M0.5–M6 features still work. `archonos ask` exits 1 with a clear message about how to configure. The `ask` workflow step fails the run cleanly (per §3.4 fail-fast).
...
```bash
# Run the full test suite (130 tests, ~8s)
pytest tests/ -v
...
│   │   └── sources/               # 8 paper-source modules
